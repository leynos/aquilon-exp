...
"""Module for testing the del cluster autostartlist command."""
...
class TestDelClusterAutoStartList(TestBrokerCommand):

    def test_100_del_rg_single_host(self):
        self.noouttest(["del_cluster_autostartlist", "--cluster", "utbvcs1b",
    ...
    def test_105_cat_utbvcs1b(self):
        ...
        self.matchclean(out, "auto_start_list", command)

    def test_105_show_utbvcs1b(self):
        command = ["show_cluster", "--cluster", "utbvcs1b"]
        out = self.commandtest(command)
        self.matchclean(out, "Member: utbhost04.aqd-unittest.ms.com Order:", command)
    ...
    def test_110_cluster_default_all(self):
        ...
        self.noouttest(["del_cluster_autostartlist", "--cluster", "utbvcs1d", "--all"])
    ...
    def test_111_rg_override(self):
        path = ["resource", "cluster", "utbvcs1d",
                ...
                "auto_start_list", "auto_start_list", "config"]
        self.check_plenary_exists(*path)
        self.noouttest(["del_cluster_autostartlist", "--cluster", "utbvcs1d",
                        "--resourcegroup", "utbvcs1das01",
    ...
    def test_115_show_utbvcs1d(self):
        ...
        out = self.commandtest(command)
        self.matchclean(out, "AutoStartList", command)
    ...
    def test_200_del_again(self):
        command = ["del_cluster_autostartlist", "--cluster", "utbvcs1b",
                   "--resourcegroup", "utbvcs1bas01",
                   "--hostname", "utbhost04.aqd-unittest.ms.com"]
        out = self.notfoundtest(command)
        self.matchoutput(out,
                         "AutoStartList auto_start_list, "
                         "resource group utbvcs1bas01 not found.",
                         command)

if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TestDelClusterAutoStartList)
