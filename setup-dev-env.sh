#!/bin/ksh
export AQDCONF="$shadow/etc/aqd.conf.dev"
...
# Python deps (pytest, pytest plugins, plugin code)
module load cloudify/eod-dev-tools/dev
export PYTHONPATH=lib:tests:$PYTHONPATH:$(dump-deps tests/test_dependencies.py)
alias pyt="pytest --disable-pytest-warnings -xv -rP"

echo http://wiki.ms.com/Aquilon/DevelopingTheBroker#How_do_I_run_a_test_broker
