...
def clean_old(installdir):
    if not os.path.isdir(installdir):
        print("Warning: '%s' is not a directory, not searching "
              "for and removing stale files." % installdir, file=sys.stderr)
        return
...
if __name__ == '__main__':
    if len(sys.argv) > 2:
        print("Takes only one argument, the install directory to prune.", file=sys.stderr)
        sys.exit(1)
    if len(sys.argv) < 2:
        print("The install directory to prune is required.", file=sys.stderr)
        sys.exit(1)
