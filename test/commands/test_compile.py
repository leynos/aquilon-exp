...
parent_dir_path = os.path.abspath(os.path.join(__file__, "../../.."))
sys.path.insert(0, os.path.join(parent_dir_path, "lib"))
...
class TestCommandCompile(unittest.TestCase):
    @staticmethod
    @mock.patch.object(aquilon.worker.commands.compile.CommandCompile, '__init__')
    def get_command_instance(mock_init):
        mock_init.return_value = None
        command = aquilon.worker.commands.compile.CommandCompile()
        return command

    @mock.patch.object(aquilon.worker.commands.compile, 'CompileKey')
    @mock.patch.object(aquilon.worker.commands.compile.CommandCompile, '_get_template_domain')
    def test_render_passes_correct_pancexclude_and_pancinclude_to_compile(
        ...
        mock_gtd.return_value = mock_template_domain
        ...
        command.render('session', 'logger', plenaries, 'domain', 'sandbox',
